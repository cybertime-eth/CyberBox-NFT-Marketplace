...
contract MarketPlaceAPI {
    ...
    function _isTokenOwner(
        ...
        address tokenAddress = Interface(marketplace).tokenAddress();
    ...
    function _isBidValid(address marketplace, Interface.Bid memory bid)
        private
        view
        returns (bool)
    {
        address paymentTokenAddress = Interface(marketplace).paymentTokenAddress();
        if (
    ...
    function getBidderTokenBid(
        ...
        uint256 tokenId,
        address bidder
    ) public view returns (Interface.Bid memory) {
        try Interface(marketplace).getBidderTokenBid(tokenId, bidder) returns (
            Interface.Bid memory bid
        ) {
            if (_isBidValid(marketplace, bid)) {
                return bid;
            } else {
                return Interface.Bid(tokenId, 0, address(0), 0);
            }
        } catch {
            return Interface.Bid(tokenId, 0, address(0), 0);
        }
    ...
    function getTokenBids(address marketplace, uint256 tokenId)
        public
        view
        returns (Interface.Bid[] memory)
    {
        Interface.Bid[] memory bids = Interface(marketplace).getTokenBids(
            tokenId
        );
        for (uint256 i; i < bids.length; i++) {
            if (!_isBidValid(marketplace, bids[i])) {
                bids[i] = Interface.Bid(tokenId, 0, address(0), 0);
            }
        }
        return bids;
    }
    ...
    function getTokenHighestBid(address marketplace, uint256 tokenId)
        ...
        returns (Interface.Bid memory)
    {
        Interface.Bid memory highestBid = Interface.Bid(
            tokenId,
            0,
            address(0),
            0
        );
        Interface.Bid[] memory bids = getTokenBids(marketplace, tokenId);
    ...
    function getTokenHighestBids(
        ...
        uint256 from,
        uint256 size
    ) external view returns (Interface.Bid[] memory) {
        Interface.Bid[] memory highestBids = Interface(marketplace)
    ...
    function getAllTokenHighestBids(address marketplace)
        ...
        returns (Interface.Bid[] memory)
    {
        Interface.Bid[] memory allHighestBids = Interface(marketplace)
